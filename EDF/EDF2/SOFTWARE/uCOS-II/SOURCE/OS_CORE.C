...
static void OS_SchedNew(void);                                /*EDF scheduling                              */
    ...
    if (OSRunning == TRUE) {
        ...
        if ((OSIntNesting == 0) && (OSLockNesting == 0)) { /* Reschedule only if all ISRs complete ... */
            ...
            if (OSPrioHighRdy != OSPrioCur) {              /* No Ctx Sw if current task is highest rdy */
                ...
                /*                              Modification                                           */
                task_log[task_log_index][0] = OSTime;
                task_log[task_log_index][1] = 1;           /*               Preempt                    */
                task_log[task_log_index][2] = OSPrioCur;
                task_log[task_log_index][3] = OSTCBHighRdy->OSTCBPrio;
                task_log[task_log_index][4] = task_log_index;
                task_log_index = task_log_index+1;
                /*                                END                                                  */
...
void  OS_Sched (void)
{
    ...
    if ((OSIntNesting == 0) && (OSLockNesting == 0)) { /* Sched. only if all ISRs done & not locked    */
        ...
        if (OSPrioHighRdy != OSPrioCur) {              /* No Ctx Sw if current task is highest rdy     */
            /*                                  Modification                                            */
            //int cur_time = OSTimeGet();
            task_log_index[task_log][0] = OSTime;
            task_log_index[task_log][1] = 2;           /*           Complete                           */
            task_log_index[task_log][2] = OSPrioCur;
            task_log_index[task_log][3] = OSTCBHighRdy->OSTCBPrio;
            task_log_index[task_log][4] = task_log_index;
            task_log_index = task_log_index+1;
            /*                                 END                                                     */
